...
把真实世界的 App 瞬间还原成可读 Java：226MB / 20 dex 的 weibo
（9.8 万个类）**5.4s** 全部落盘；353MB / 59 dex 的飞书 **9.2s**。
...
[Releases](https://github.com/ejfkdev/ddc/releases) 附带各平台预编译二进制
（Linux x86_64/aarch64、macOS x86_64/Apple 芯片、Windows x86_64）——
**裸二进制，无压缩包**；非 macOS 平台已经 UPX 压缩。打 tag 即触发
构建，二进制版本号等于 tag 版本。
...
```bash
cargo test          # 55 个：解码/端到端 + CLI + 子命令集成
```
...
[MIT](LICENSE) © ejfkdev。依赖 [`jdc-core`](https://crates.io/crates/jdc-core)
同为 MIT。
